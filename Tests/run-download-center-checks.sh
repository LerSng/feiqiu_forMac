...
build_root="$(mktemp -d "${TMPDIR:-/tmp}/feiq-download-center-checks.XXXXXX")"
...
for check in DownloadCenterChecks FileTransferCenterChecks; do
    echo "Running $check"
...
echo "All download center checks passed"
