...
build_root="$(mktemp -d "${TMPDIR:-/tmp}/feiq-transfer-checks.XXXXXX")"
...
for check in FileTransferCenterChecks FileTransferIOChecks FileDownloadChecks IncomingImageRepositoryChecks InlineImageProtocolChecks; do
    echo "Running $check"
    xcrun swiftc "${flags[@]}" "${sources[@]}" "$project_root/Tests/$check.swift" -lsqlite3 -o "$build_root/$check"
    "$build_root/$check"
done
...
echo "All file transfer checks passed"
