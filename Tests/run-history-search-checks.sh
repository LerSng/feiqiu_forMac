...
build_root="$(mktemp -d "${TMPDIR:-/tmp}/feiq-history-search-checks.XXXXXX")"
...
for check in HistorySearchChecks HistoryArchiveChecks; do
    echo "Running $check"
...
echo "All history search and archive checks passed"
